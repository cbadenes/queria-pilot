...
class Conversation:
    ...
    def prepare_json_string(s):
        # Encuentra todos los segmentos de texto entre comillas
        parts = re.split(r'("[^"\\]*(?:\\.[^"\\]*)*")', s)
        for i in range(1, len(parts), 2):  # Solo los segmentos que están entre comillas
            parts[i] = parts[i].replace('"', '\\"')  # Escapa solo las comillas internas
        corrected_string = ''.join(parts)
